...
while camera.isOpened():
    ...
    if isBgCaptured == 1:  # this part wont run until background captured
        ...
        if length > 0:
            ...
            cv2.drawContours(raw_frame, [res], 0, (0, 255, 0), 2)
            cv2.drawContours(raw_frame, [hull], 0, (0, 0, 255), 3)

            isFinishCal,num = calculateFingers(res,raw_frame)
            number = str(num + 1)
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(raw_frame, number, (120, 120), font, 2, (255, 0, 255), 2)


        cv2.imshow('output', raw_frame)
        if triggerSwitch == True:
            if oldnum != num:
                if num == 0:
                    oldnum = 0;

                    win32api.keybd_event(116, 0, 0, 0)  # v键位码是86
                    win32api.keybd_event(116, 0, win32con.KEYEVENTF_KEYUP, 0)

                elif num == 1:
                    oldnum=1
                    win32api.keybd_event(39, 0, 0, 0)  # v键位码是86
                    win32api.keybd_event(39, 0, win32con.KEYEVENTF_KEYUP, 0)
                    #time.sleep(5)
                elif num == 2:
                    oldnum = 2
                    #win32api.keybd_event(37, 0, 0, 0)  # v键位码是86
                    ...
                    oldnum=4
                    win32api.keybd_event(27, 0, 0, 0)  # v键位码是86
